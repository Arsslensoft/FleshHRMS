...
namespace PHRMS.Data {
    ...
    public sealed class PhoneAttribute : RegexAttributeBase {
        static readonly Regex regex = new Regex(@"\+\d{1,3}\s\d\d\d\d\d\d\d\d", DefaultRegexOptions);
    ...
    public sealed class ZipCodeAttribute : RegexAttributeBase {
        static Regex regex = new Regex(@"^[0-9][0-9][0-9][0-9]$", DefaultRegexOptions);
    ...
    {
        static Regex regex = new Regex(@"^[0-9][0-9][0-9][0-9][0-9][0-9][0-9][0-9]$", DefaultRegexOptions);
        const string Message = "The {0} field is not a valid CIN code.";
        public CINAttribute()
            : base(regex, Message, DataType.Url)
        {
        }
