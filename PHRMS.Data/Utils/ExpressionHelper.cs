...
namespace PHRMS.Utils
{
    ...
    public class ExpressionHelper
    {
        ...
        public static Expression<Func<TPropertyOwner, bool>> GetValueEqualsExpression<TPropertyOwner, TProperty>(
            Expression<Func<TPropertyOwner, TProperty>> getPropertyExpression, TProperty constant)
        {
            Expression equalExpression = Expression.Equal(getPropertyExpression.Body,
                Expression.Convert(
                    Expression.Field(Expression.Constant(new ValueHolder<TProperty>(constant)), "value"),
                    getPropertyExpression.Body.Type));
            return Expression.Lambda<Func<TPropertyOwner, bool>>(equalExpression,
                getPropertyExpression.Parameters.Single());
        }

        /// <summary>
        ///     Returns an instance of the EntityTraits class that encapsulates operations to obtain and set the primary key value
        ///     of a given entity.
        /// </summary>
        ...
        public static EntityTraits<TPropertyOwner, TProperty> GetEntityTraits<TOwner, TPropertyOwner, TProperty>(
            ...
            Action<TPropertyOwner, TProperty> setPropertyAction)
        {
            object traits = null;
            if (!TraitsCache.TryGetValue(owner.GetType(), out traits))
            {
                traits = new EntityTraits<TPropertyOwner, TProperty>(getPropertyExpression.Compile(),
                    setPropertyAction ?? GetSetValueActionExpression(getPropertyExpression).Compile(),
                    GetHasValueFunctionExpression(getPropertyExpression).Compile());
                TraitsCache[owner.GetType()] = traits;
            }
            return (EntityTraits<TPropertyOwner, TProperty>) traits;
        }

        private static Expression<Action<TPropertyOwner, TProperty>> GetSetValueActionExpression
            <TPropertyOwner, TProperty>(Expression<Func<TPropertyOwner, TProperty>> getPropertyExpression)
        {
            var body = (MemberExpression) getPropertyExpression.Body;
            ...
            var assignPropertyValueExpression = Expression.Assign(body, propertyValueParameter);
            return Expression.Lambda<Action<TPropertyOwner, TProperty>>(assignPropertyValueExpression, thisParameter,
                propertyValueParameter);
        }

        private static Expression<Func<TPropertyOwner, bool>> GetHasValueFunctionExpression<TPropertyOwner, TProperty>(
    ...
    public class EntityTraits<TEntity, TPrimaryKey>
    {
        ...
        /// <summary>
        ///     A function that determines whether the given entity has a primary key assigned (the primary key is not null).
        ///     Always returns true if the primary key is a non-nullable value type.
        /// </summary>
        /// <returns></returns>
