...
namespace PHRMS.Widgets
{
    public partial class LeaveByStatusWidget : UserControl, IDashboardWidget
    {
        ...
        {
            Controls.Add(GetLeavesByStatusChartControl(value));
        }
        ...
        public ChartControl GetLeavesByStatusChartControl(BoardViewModel value)
        {
            ...
            // Create a pie series.
            var series1 = new Series("Congés par status", ViewType.Pie);
        ...
        private IEnumerable<PercentageStats<LeaveStatus>> CacheStats(BoardViewModel bv)
        {
            IEnumerable<PercentageStats<LeaveStatus>> result = null;
            var filename = Application.StartupPath + "\\Data\\leaveSTstats.dat";
            if (File.Exists(filename))
            {
                ...
                else // deserialize
                {
                    ...
                    result = (IEnumerable<PercentageStats<LeaveStatus>>) bformatter.Deserialize(stream);
                    stream.Close();
                }
            }
            else
