...
namespace PHRMS.Services
{
    ...
    {
        private readonly IDictionary<ModuleType, WeakReference> modulesCache;
        private readonly IDictionary<ModuleType, IDictionary<Guid, WeakReference>> modulesIdentityCache;
        private readonly IServiceContainer serviceContainer;

        public ModuleLocator(IServiceContainer serviceContainer)
        {
            this.serviceContainer = serviceContainer;
            modulesCache = new Dictionary<ModuleType, WeakReference>();
            modulesIdentityCache = new Dictionary<ModuleType, IDictionary<Guid, WeakReference>>();
        }
        ...
        public object GetModule(ModuleType moduleType, Guid keyParameter)
        {
            ...
            return GetModuleCore(identityCache, keyParameter, moduleType,
                (moduleTypeName, parameter) => activator.CreateModule(moduleTypeName), null);
        }
        ...
        public object GetModule(ModuleType moduleType, object viewModel)
        {
            if (moduleType == ModuleType.Unknown) return null;
            var activator = serviceContainer.GetService<IModuleActivator>();
            return GetModuleCore(modulesCache, moduleType,
                (moduleTypeName, parameter) => activator.CreateModule(moduleTypeName, parameter), viewModel);
        }
        ...
        private void ClearCore<TKey>(IDictionary<TKey, WeakReference> cache, object module)
        {
            var key = default(TKey);
            foreach (var item in cache)
            {
                if (!Equals(item.Value.Target, module)) continue;
                key = item.Key;
                break;
            }
            cache.Remove(key);
    ...
    internal class ReportLocator : IReportLocator
    {
        private readonly IDictionary<object, WeakReference> reportsCache;
        private readonly IServiceContainer serviceContainer;

        public ReportLocator(IServiceContainer serviceContainer)
        {
            this.serviceContainer = serviceContainer;
            reportsCache = new Dictionary<object, WeakReference>();
        }
        ...
        {
            if (ReferenceEquals(reportKey, null))
            {
                return;
            }
            ...
            if (reportsCache.TryGetValue(reportKey, out report))
            {
                ...
                var disposable = report.Target as IDisposable;
                if (disposable != null)
                {
                    disposable.Dispose();
                }
        ...
        private object GetReportCore(object reportKey, Func<string, object> activatorRoutine)
        {
            if (ReferenceEquals(reportKey, null))
            {
                return null;
            ...
            {
                var resolver = serviceContainer.GetService<IModuleTypesResolver>();
                var report = activatorRoutine(resolver.GetReportTypeName(reportKey));
