...
namespace PHRMS.ViewModels
{
    ...
    public partial class EmployeeViewModel : SingleObjectViewModel<Employee, long, IPhexonDbUnitOfWork>
    {
        ...
        ///     This constructor is declared protected to avoid undesired instantiation of the EmployeeViewModel type without the
        ///     POCO proxy factory.
        /// </summary>
        ...
        {
            EmployeeAssignedTasksLookUp = CreateLookUpCollectionViewModel(x => x.Tasks, x => x.AssignedEmployeeId,
                (x, m) => x.AssignedEmployee = m, key);
            EmployeeOwnedTasksLookUp = CreateLookUpCollectionViewModel(x => x.Tasks, x => x.OwnerId,
                (x, m) => x.Owner = m, key);
            EmployeeEvaluationsLookUp = CreateLookUpCollectionViewModel(x => x.Evaluations, x => x.EmployeeId,
                (x, m) => x.Employee = m, key);
            EmployeeShiftsLookUp = CreateLookUpCollectionViewModel(x => x.Shifts, x => x.EmployeeId,
                (x, m) => x.Employee = m, key);
        }
    }
}