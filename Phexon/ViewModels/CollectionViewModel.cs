...
{
    /// <summary>
    ///     The base class for a POCO view models exposing a collection of entities of a given type and CRUD operations against
    ///     these entities.
    ///     This is a partial class that provides extension point to add custom properties, commands and override methods
    ...
    public partial class CollectionViewModel<TEntity, TPrimaryKey, TUnitOfWork> :
        CollectionViewModelBase<TEntity, TPrimaryKey, TUnitOfWork>
        where TEntity : class
        where TUnitOfWork : IUnitOfWork
    {
        ...
        /// <param name="newEntityInitializerFactory">
        ///     An optional parameter that provides a function to create an entity
        ///     initializer. This parameter is used in the detail collection view models when creating a single object view model
        ///     for a new entity.
        /// </param>
        public CollectionViewModel(IUnitOfWorkFactory<TUnitOfWork> unitOfWorkFactory,
            Func<TUnitOfWork, IRepository<TEntity, TPrimaryKey>> getRepositoryFunc,
            Func<IEntityInitializer<TEntity, TUnitOfWork>> newEntityInitializerFactory = null)
    ...
    /// <summary>
    ///     The base class for POCO view models exposing a collection of entities of a given type and CRUD operations against
    ///     these entities.
    ...
    /// <typeparam name="TPrimaryKey">A primary key value type.</typeparam>
    /// <typeparam name="TUnitOfWork">A unit of work type.</typeparam>
    public abstract class CollectionViewModelBase<TEntity, TPrimaryKey, TUnitOfWork> :
        ReadOnlyCollectionViewModel<TEntity, TUnitOfWork>, IDocumentContent
        where TEntity : class
        where TUnitOfWork : IUnitOfWork
    {
        private readonly Func<IEntityInitializer<TEntity, TUnitOfWork>> newEntityInitializerFactory;
        ...
        /// <param name="newEntityInitializerFactory">
        ///     An optional parameter that provides a function to create an entity
        ///     initializer. This parameter is used in detail collection view models when creating a single object view model for a
        ///     new entity.
        /// </param>
        protected CollectionViewModelBase(IUnitOfWorkFactory<TUnitOfWork> unitOfWorkFactory,
            Func<TUnitOfWork, IRepository<TEntity, TPrimaryKey>> getRepositoryFunc,
            Func<IEntityInitializer<TEntity, TUnitOfWork>> newEntityInitializerFactory)
            : base(unitOfWorkFactory, getRepositoryFunc)
        {
            this.newEntityInitializerFactory = newEntityInitializerFactory;
        }

        protected new IRepository<TEntity, TPrimaryKey> Repository
        {
            get { return (IRepository<TEntity, TPrimaryKey>) base.Repository; }
        }
        ...
        ///     Since CollectionViewModelBase is a POCO view model, an instance of this class will also expose the NewCommand
        ///     property that can be used as a binding source in views.
        /// </summary>
        public virtual void New()
        {
            var document =
                CreateDocument(newEntityInitializerFactory != null
                    ? newEntityInitializerFactory()
                    : new DefaultEntityInitializer<TEntity, TUnitOfWork>());
            if (document != null)
                document.Show();
        }
        ...
        ///     Since CollectionViewModelBase is a POCO view model, an instance of this class will also expose the EditCommand
        ///     property that can be used as a binding source in views.
        /// </summary>
        ...
        ///     Since CollectionViewModelBase is a POCO view model, this method will be used as a CanExecute callback for
        ///     EditCommand.
        /// </summary>
        ...
        ///     Since CollectionViewModelBase is a POCO view model, an instance of this class will also expose the DeleteCommand
        ///     property that can be used as a binding source in views.
        /// </summary>
        /// <param name="entity">An entity to edit.</param>
        ...
        {
            if (
                MessageBoxService.Show(string.Format(CommonResources.Confirmation_Delete, "entité"),
        ...
        ///     Since CollectionViewModelBase is a POCO view model, instance of this class will also expose the SaveCommand
        ///     property that can be used as a binding source in views.
        /// </summary>
        ...
        ///     Since CollectionViewModelBase is a POCO view model, an instance of this class will also expose the
        ///     UpdateSelectedEntityCommand property that can be used as a binding source in views.
        /// </summary>
        [Display(AutoGenerateField = false)]
        ...
        protected virtual TEntity OnEntityChanged(TPrimaryKey key)
        {
            ...
            entity = Repository.Reload(entity);
            if (FilterExpression != null && !new[] {entity}.AsQueryable().Any(FilterExpression))
            {
                Repository.UnitOfWork.Detach(entity);
            }
            else
            {
                var index = Repository.Local.IndexOf(entity);
                if (index >= 0)
        ...
        {
            var entity = Repository.Local.FirstOrDefault(x => Equals(Repository.GetPrimaryKey(x), key));
            if (entity != null)
            {
                Repository.Remove(entity);
                Repository.UnitOfWork.Detach(entity);
        ...
        protected TEntity FindNewEntity(TPrimaryKey key)
        {
            ...
            return
                entities.Where(ExpressionHelper.GetValueEqualsExpression(Repository.GetPrimaryKeyExpression, key))
                    .FirstOrDefault();
        }
        ...
        protected TEntity FindEntity(TPrimaryKey key)
        {
            ...
            return
                Repository.Local.AsQueryable()
                    .FirstOrDefault(ExpressionHelper.GetValueEqualsExpression(Repository.GetPrimaryKeyExpression, key)) ??
                FindNewEntity(key);
        }
        ...
        private void ShowDocument(TPrimaryKey key)
        {
            var document = FindEntityDocument(key) ?? CreateDocument(key);
            if (document != null)
                document.Show();
        }
        ...
        protected IDocument FindEntityDocument(TPrimaryKey key)
        {
            ...
            foreach (var document in DocumentManagerService.Documents)
            {
                ...
                if (entityViewModel != null && Equals(entityViewModel.PrimaryKey, key))
                    return document;
            }
            return null;
        }
