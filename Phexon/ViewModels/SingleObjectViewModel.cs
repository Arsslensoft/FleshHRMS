...
{
    /// <summary>
    ///     The base class for POCO view models exposing a single entity of a given type and CRUD operations against this
    ///     entity.
        ...
        where TUnitOfWork : IUnitOfWork
    {
            ...
            Func<TEntity, object> getEntityDisplayNameFunc)
        {
            ...
            if (this.IsInDesignMode())
                Entity = Repository.Local.FirstOrDefault();
            else
        ...
        ///     Since SingleObjectViewModelBase is a POCO view model, this property will raise INotifyPropertyChanged.PropertyEvent
        ///     when modified so it can be used as a binding source in views.
        /// </summary>
        /// <returns></returns>
        ...
        internal IMessageBoxService MessageBoxService
        {
        ...
        ///     Since SingleObjectViewModelBase is a POCO view model, an instance of this class will also expose the UpdateCommand
        ...
        ///     Since SingleObjectViewModelBase is a POCO view model, an instance of this class will also expose the SaveCommand
        ///     property that can be used as a binding source in views.
        /// </summary>
        [Command]
        public virtual bool Save()
        {
            try
            {
                ...
                Messenger.Default.Send(new EntityMessage<TEntity>(Entity,
                    isNewEntity ? EntityMessageType.Added : EntityMessageType.Changed));
                Reload();
        ...
        ///     Since SingleObjectViewModelBase is a POCO view model, an instance of this class will also expose the
        ///     SaveAndCloseCommand property that can be used as a binding source in views.
        /// </summary>
        ...
        ///     Since SingleObjectViewModelBase is a POCO view model, this method will be used as a CanExecute callback for
        ///     SaveCommand.
        /// </summary>
        ...
        ///     Since SingleObjectViewModelBase is a POCO view model, an instance of this class will also expose the DeleteCommand
        ///     property that can be used as a binding source in views.
        /// </summary>
        public virtual void Delete()
        {
            if (
                MessageBoxService.Show(string.Format(CommonResources.Confirmation_Delete, typeof(TEntity).Name),
                    CommonResources.Confirmation_Caption, MessageBoxButton.YesNo) != MessageBoxResult.Yes)
                return;
            try
            {
        ...
        ///     Since SingleObjectViewModelBase is a POCO view model, an instance of this class will also expose the ResetCommand
        ///     property that can be used as a binding source in views.
        /// </summary>
        public void Reset()
        {
            var confirmationResult = MessageBoxService.Show(CommonResources.Confirmation_Reset,
                CommonResources.Confirmation_Caption, MessageBoxButton.OKCancel);
        ...
        ///     Since SingleObjectViewModelBase is a POCO view model, an instance of this class will also expose the CloseCommand
        ///     property that can be used as a binding source in views.
        /// </summary>
        public void Close()
        {
        ...
        protected virtual void OnEntityMessage(EntityMessage<TEntity> x)
        {
            ...
            if (x.MessageType == EntityMessageType.Deleted && Equals(Repository.GetPrimaryKey(x.Entity), PrimaryKey))
                Close();
        }
        ...
        {
            var initializer = parameter as IEntityInitializer<TEntity, TUnitOfWork>;
            if (initializer != null)
            {
                Entity = CreateEntity();
        ...
        protected virtual bool TryClose()
        {
            if (HasValidationErrors())
            {
                var warningResult = MessageBoxService.Show(CommonResources.Warning_SomeFieldsContainInvalidData,
                    CommonResources.Warning_Caption, MessageBoxButton.OKCancel);
                return warningResult == MessageBoxResult.OK;
            }
            if (!NeedSave()) return true;
            var result = MessageBoxService.Show(CommonResources.Confirmation_Save, CommonResources.Confirmation_Caption,
        ...
        private void InitializeEntity(IEntityInitializer<TEntity, TUnitOfWork> entityInitializer)
        {
        ...
        protected virtual CollectionViewModel<TDetailEntity, TDetailPrimaryKey, TUnitOfWork>
            CreateLookUpCollectionViewModel<TDetailEntity, TDetailPrimaryKey, TForeignKey>(
            ...
            Action<TDetailEntity, TEntity> setMasterEntityAction, TForeignKey masterEntityKey)
            where TDetailEntity : class
        {
            var lookUpViewModel =
                ViewModelSource.Create(
                    () =>
                        new CollectionViewModel<TDetailEntity, TDetailPrimaryKey, TUnitOfWork>(UnitOfWorkFactory,
                            getRepositoryFunc,
                            () => CreateNavigationPropertyInitializer(setMasterEntityAction, masterEntityKey)));
            lookUpViewModel.SetParentViewModel(this);
            lookUpViewModel.FilterExpression = ExpressionHelper.GetValueEqualsExpression(foreignKeyExpression,
                masterEntityKey);
            return lookUpViewModel;
        }

        protected virtual ReadOnlyCollectionViewModel<TDetailEntity, TUnitOfWork>
            CreateReadOnlyLookUpCollectionViewModel<TDetailEntity, TForeignKey>(
            Func<TUnitOfWork, IReadOnlyRepository<TDetailEntity>> getRepositoryFunc,
            Expression<Func<TDetailEntity, TForeignKey>> foreignKeyExpression, TForeignKey masterEntityKey)
            where TDetailEntity : class
        {
            var lookUpViewModel =
                ViewModelSource.Create(
                    () =>
                        new ReadOnlyCollectionViewModel<TDetailEntity, TUnitOfWork>(UnitOfWorkFactory, getRepositoryFunc));
            lookUpViewModel.SetParentViewModel(this);
            lookUpViewModel.FilterExpression = ExpressionHelper.GetValueEqualsExpression(foreignKeyExpression,
                masterEntityKey);
            return lookUpViewModel;
        ...
        private IEntityInitializer<TDetailEntity, TUnitOfWork> CreateNavigationPropertyInitializer
            <TDetailEntity, TForeignKey>(Action<TDetailEntity, TEntity> setMasterEntityAction,
                TForeignKey masterEntityKey) where TDetailEntity : class
        {
            return
                new EntityNavigationPropertyInitializer<TDetailEntity, TEntity, TPrimaryKey, TUnitOfWork>(
                    (TPrimaryKey) (object) masterEntityKey, setMasterEntityAction, getRepositoryFunc);
        }

        protected virtual IList<TLookUpEntity> GetLookUpEntities<TLookUpEntity>(
            Func<TUnitOfWork, IReadOnlyRepository<TLookUpEntity>> getRepositoryFunc) where TLookUpEntity : class
        {
